var PEEP_METADATA = {
	...
	random: { frame: Math.floor(Math.random() * 100) % 7, color: "#FF5E5E" }
};
...
function storeDb() {
	...
	(async () => {
		const rawResponse = await fetch("http://localhost:3000/insert", {
			method: 'POST',
	...
	// 	let iUrl="http://localhost/trust/crypto/insert.php?";
	// 	iUrl+="input="
	// 	iUrl+=input;
	// 	iUrl+="&output="
	// 	iUrl+=output;
...
function Logic_random() {
	...
	self.play = async function () {
		...
		await fetch('http://localhost:3000/random', {
			...
			.then(data => {
				randomResp = data.random;
				storeDb();
			})
			.catch(err => {
				console.log(err);
			})
		return (randomResp ? PD.COOPERATE : PD.CHEAT);
	};
